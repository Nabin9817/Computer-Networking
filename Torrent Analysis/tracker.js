...
export const getPeers = (torrent, callback) => {
  ...
  socket.on("message", (response) => {
    console.log("ddddddddddddddcheck");
    if (respType(response) === "connect") {

      // 2. receive and parse connect response
      console.log("cccccccheck");
      const connResp = parseConnResp(response);
      console.log("bbbbbbb connResp: ", connResp)
      // 3. send announce request
      console.log("aaaaaaaaaaa connResp.connectionId: ", connResp.connectionId);
      ...
      console.log("heeeeeee:", announceResp.peers)
      callback(announceResp.peers);
    }
  });
};

function udpSend(socket, message, rawUrl, callback=(err)=>{console.log('errror eeee',err)}) {
...
console.log('build connection request bhitra');
  return buf;
};
